  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="relative mx-auto max-w-2xl px-4 pb-24 pt-24 md:pt-28">
        ...
        <div className="mb-6 rounded-3xl border border-white/[0.08] bg-white/[0.05] p-6 backdrop-blur-2xl md:p-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[10px] font-medium uppercase tracking-[0.25em] text-white/35">
                  {user.role}
                </div>
            ...
            <div className="flex flex-col items-end gap-2">
              <div className="rounded-xl bg-white/[0.08] px-4 py-2 text-right backdrop-blur-xl">
                <div className="text-[9px] font-medium uppercase tracking-[0.2em] text-white/40">Үлдэгдэл</div>
                <div className="mt-0.5 text-base font-black text-white">{user.balance.toLocaleString()}₮</div>
              </div>
              <button
                onClick={logout}
                className="text-[9px] font-medium uppercase tracking-[0.2em] text-white/25 transition-colors hover:text-red-400"
              >
                Гарах
              </button>
            </div>
          ...
          {activeBookings.length > 0 && (
            <div className="mt-5 space-y-2">
              {activeBookings.map((b) => (
                <div key={b.id} className="flex items-center justify-between rounded-2xl border border-green-500/20 bg-green-500/[0.07] px-4 py-3 backdrop-blur-xl">
                  ...
                  <button
                    ...
                    className="text-[9px] text-white/25 transition-colors hover:text-red-400 disabled:opacity-30"
                  >
              ...
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-2 rounded-2xl border py-3 text-[11px] font-semibold transition-all duration-300 backdrop-blur-xl ${
                  tab === t
                    ...
                    : "border-white/[0.06] bg-white/[0.03] text-white/40 hover:bg-white/[0.07] hover:text-white/70"
                }`}
              >
                {t === "history" ? "Түүх" : "Дуртай"}
                {count > 0 && (
                  <span className={`rounded-full px-1.5 py-0.5 text-[8px] font-medium ${
                    tab === t ? "bg-white/20 text-white/70" : "bg-white/[0.06] text-white/30"
                  }`}>{count}</span>
                )}
              </button>
            );
        ...
        {/* ── HISTORY ── */}
        {tab === "history" && (
          <div className="rounded-3xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-2xl overflow-hidden">
                ...
                <div key={b.id} className="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-white/[0.03]">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="mono text-[9px] text-white/25">{b.code}</span>
                    ...
                    <div className="mt-0.5 text-[13px] font-semibold text-white/90">
                      ...
                      <span className="ml-2 text-[11px] font-normal text-white/35">{b.center.name}</span>
                    </div>
                    <div className="mono mt-0.5 text-[9px] text-white/25">
            ...
            {totalHistoryPages > 1 && (
              <div className="flex items-center justify-between border-t border-white/[0.06] px-5 py-3">
                <button
                  ...
                  className="text-[10px] font-medium text-white/30 transition-colors hover:text-white disabled:opacity-20"
                >
                  ← Өмнөх
                </button>
                <span className="mono text-[10px] text-white/25">{historyPage} / {totalHistoryPages}</span>
                <button
          ...
          <div className="rounded-3xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-2xl overflow-hidden">
            {favorites.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-20 text-center">
                ...
                <p className="text-sm font-medium text-white/30">Дуртай центр байхгүй</p>
                <p className="text-[11px] text-white/20">Центрийн хуудсан дээрх ♡ дарж нэмнэ үү</p>
                <Link
                  href="/"
                  className="mt-3 rounded-xl bg-white/[0.08] px-5 py-2.5 text-[11px] font-semibold text-white/60 transition hover:bg-white/[0.14] hover:text-white"
                >
                  Центрүүд харах →
                </Link>
              </div>
                  ...
                  return (
                    <div key={c.id} className="group flex items-center gap-4 px-5 py-4 transition-colors hover:bg-white/[0.03]">
                      ...
                      <Link href={`/centers/${c.id}`} className="min-w-0 flex-1">
                        <div className="text-[9px] font-medium text-white/35">{c.district}</div>
                        <div className="truncate text-[13px] font-semibold text-white/90">{c.name}</div>
                        <div className="mono mt-0.5 flex items-center gap-2.5 text-[9px] text-white/30">
                      ...
                      <button
                        ...
                        className="shrink-0 rounded-lg p-1.5 text-white/20 transition-colors hover:bg-red-500/10 hover:text-red-400 disabled:opacity-30"
                      >
